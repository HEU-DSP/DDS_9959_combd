...
|    PD7    | SPI1_MOSI   | `SPI_9959_DIO0`          | SPI1 MOSI               | **双线模式**：TIM2_CH1 上升沿 → DMA 搬运 → SPI1 发送数据，AD9959 SDIO0                               |
|    PD6    | SPI3_MOSI   | `SPI_9959_DIO1`          | SPI3 MOSI (TX Only)     | **双线模式**：TIM2_CH1 上升沿 → DMA 搬运 → SPI3 发送数据，AD9959 SDIO1                               |
|    PD5    | GPIO_Output | `IO_9959_DIO2`           | GPIO 输出               | AD9959 SDIO2，**预留**                                                                                 |
|    PC7    | TIM8_CH2    | `SPI_9959_DIO3`          | TIM8 CH2 Output Compare | 校准延迟后输出脉冲给 AD9959 DIO3，刷新 IO 寄存器                                                             |
|    PC6    | TIM8_CH1    | `SYNC_9959_IO_UPDATE`    | TIM8 CH1 Output Compare | 校准延迟后输出脉冲给 AD9959 IO_UPDATE，刷新 IO 寄存器                                                        |
...
> **TIM4 时序测量原理**：PE0 外部连接 PA0，TIM4 ETR 检测 TIM2_CH1 的上升沿，**硬件自动清零 TIM4 CNT**；PB6/PB7 直连 SPI1 硬件 NSS (PA15)，分别在 CS↓ 和 CS↑ 时捕获 TIM4 计数值。两次捕获值即为"触发 DMA → SPI 开始通信"和"触发 DMA → SPI 通信结束"的时间，**供调试时选择合适的 TIM8 比较值**，以确定 IO_UPDATE 和 DIO3 脉冲的延迟。
...
## OCR 控制接口（两片 74HC595 级联）

### MCU → 595 控制信号

| STM32 引脚 | 信号功能    | GPIO_Label   | 外设复用            | 连接                                        | 备注                                     |
| :--------: | :---------- | :----------- | :------------------ | :------------------------------------------ | :--------------------------------------- |
|    PE2    | GPIO_Output | `OCR_STCP` | GPIO 输出           | 两片 595 的 STCP (RCLK)                     | 存储时钟：将移位寄存器数据锁存到并行输出 |
|    PE3    | USART10_TX  | `OCR_DS`   | USART10 TX (半双工) | 第一片 595 的 DS (SER)                      | 串行数据输入 (USART10 帧 → 移位寄存器)  |
|    PE4    | GPIO_Output | `OCR_SHCP` | GPIO 输出           | 两片 595 的 SHCP (SRCLK)                    | 移位时钟：每个脉冲将 DS 移入移位寄存器   |
|    PE5    | GPIO_Output | `OCR_NMR`  | GPIO 输出           | 两片 595 的\$\overline{\text{MR}}\$ (SRCLR) | 主复位 (低有效)，同时清空两片移位寄存器  |
|    PE6    | GPIO_Output | `OCR_NOE`  | GPIO 输出           | 两片 595 的\$\overline{\text{OE}}\$         | 输出使能 (低有效)，同时控制两片并行输出  |

### 数据链路（级联方式）

```
USART10_TX (PE3) ──► DS ──► [ 595 #1 ] ──► Q7'(QH') ──► DS ──► [ 595 #2 ]
                              │  8 位并行输出               │  8 位并行输出
                              ▼                            ▼
```

> USART10 工作在**半双工单线模式**，每次发送一帧（16 位），帧数据通过 DS→SHCP 逐位移入第一片 595。第一片的串行溢出 (Q7') 级联进入第二片 DS。数据全部移入后，STCP 脉冲将两片移位寄存器同时锁存到并行输出。
...
| :-: | :------------------ | :---------- | :------------------ |
| 0 | NC                  | —          | 未连接              |
| 1 | `LEDSTBY`         | MCU → 板级 | 待机指示灯          |
| 2 | `LED ANALOGREADY` | MCU → 板级 | 模拟前端就绪指示灯  |
| 3 | `LEDMODREADY`     | MCU → 板级 | 调制/模式就绪指示灯 |
| 4 | `LEDCH0TRANSMIT`  | MCU → 板级 | CH0 发射指示灯      |
| 5 | `LEDCH1TRANSMIT`  | MCU → 板级 | CH1 发射指示灯      |
| 6 | `LEDCH2TRANSMIT`  | MCU → 板级 | CH2 发射指示灯      |
| 7 | `LEDCH3TRANSMIT`  | MCU → 板级 | CH3 发射指示灯      |
...
| :-: | :---------------- | :--------- | :-------------------------- |
| 0 | NC                | —         | 未连接                      |
| 1 | `DDSPWREN1V8D`  | MCU → DDS | DDS 数字 1.8V 电源使能      |
| 2 | `DDSPWREN1V8A`  | MCU → DDS | DDS 模拟 1.8V 电源使能      |
| 3 | `DDSMASTERRST`  | MCU → DDS | DDS 主复位                  |
| 4 | `DDSPDN`        | MCU → DDS | DDS 掉电控制                |
| 5 | `DDSPWREN_3V3D` | MCU → DDS | DDS 数字 3.3V 电源使能      |
| 6 | `DDSCLKMODE33`  | MCU → DDS | DDS REF_CLK 模式选择 (3.3V) |
| 7 | NC                | —         | 未连接                      |
...
| STM32 引脚 | 信号功能    | 备注                       |
| :--------: | :---------- | :------------------------- |
|    PH0    | RCC_OSC_IN  | HSE 外部时钟输入 (25 MHz) |
